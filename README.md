...
If you are using the middleware with a Graphql server implemented using Koa, the partial errors from resovers are also logged by this middleware. The response in case of the partial error is returned as 200 with errors in the body for a graphql API. The middleware will look for a key errors in the response and will log it in error object.
